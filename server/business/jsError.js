...
exports.create_error = (data) => {
    var temp = new ErrorModel(data);
    temp.save(function (err, r) {
        if (err) {
            console.error(err);
        }
...
exports.errorList = async (req) => {
    ...
    resJson.TotalCount = await ErrorModel.find(tempCon).countDocuments();
    if (resJson.TotalCount) {
        resJson.List = await ErrorModel.find(tempCon).sort({
            "createTime": -1
        }).skip((req.body.pageIndex - 1) * req.body.pageSize).limit(req.body.pageSize);
    }
    return resJson;
